      ...
      <table className="w-full text-left text-slate-900 border-separate border-spacing-0">
        ...
        <tbody className="divide-y divide-slate-100">
          {contributions.map((c: any) => {
            ...
            // BUSCAMOS EN TODOS LOS CAMPOS POSIBLES PARA LA DESCRIPCIÓN
            const displayDesc = c.description || c.concept || c.desc || "—";
            ...
            return (
              <tr key={c.id} className="hover:bg-slate-50/50 transition-colors">
                ...
                <td className="px-6 py-5">
                  <span className="px-2 py-1 rounded-[6px] text-[9px] font-black border bg-slate-50 text-slate-500 uppercase">
                    {displayType}
                  </span>
                </td>
                ...
                <td className="px-6 py-5 text-right font-black font-mono">{Number(c.risk_adjusted_value).toLocaleString()}</td>
                <td className="px-6 py-5 text-right font-black text-emerald-600 font-mono">{percentage}%</td>
                <td className="px-6 py-5 text-center">
                  <button onClick={() => handleDelete(c)} className="p-2 text-slate-300 hover:text-red-500"><Trash2 className="w-4 h-4" /></button>
                </td>
              </tr>
            );
          })}
