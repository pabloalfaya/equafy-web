    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-50">
        <div className="text-center">
          ...
          <p className="text-slate-600">Loading Equily...</p>
        </div>
      </div>
    );
  }
            ...
            {fetchError ? (
              <>
                ...
                <p className="text-xs text-slate-500">
                  If using Supabase RLS, ensure you have a SELECT policy on the &quot;projects&quot; table.
                </p>
              </>
            ) : (
              <p className="text-slate-600">
                No projects found. Create one in Supabase to start.
              </p>
            )}
          </div>
        ) : (
            ...
            <div className="mb-10 flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Equity distribution based on risk-adjusted contributions
                </p>
              </div>
              <button
                type="button"
              ...
              <div className="mb-4 flex items-center justify-between">
                ...
                {contributions.length > 0 && (
                  <span className="rounded-full bg-emerald-50 px-3 py-1 text-sm font-medium text-emerald-fintech">
                    {contributions.length} entr{contributions.length !== 1 ? "ies" : "y"}
                  </span>
                )}
              </div>
